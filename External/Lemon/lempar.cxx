...
#define LEMON_SUPER as the name of a class which overrides
lemon_base<TokenType>. The parser will be implemented in terms of that. add a
%code section to instantiate it.
 */
...
// this is here so you can do something like Parse(void *, int, my_token &&) or
// (... const my_token &)
...
template <> struct yy_fix_type<void>
{
    typedef struct
    {
    } type;
...
template <class T> typename yy_fix_type<T>::type &yy_cast(void *vp)
{
    typedef typename yy_fix_type<T>::type TT;
    return *(TT *)vp;
}

template <class T> void yy_destructor(void *vp)
{
    typedef typename yy_fix_type<T>::type TT;
    ((TT *)vp)->~TT();
}

template <class T> void yy_destructor(T &t) { t.~T(); }

template <class T> void yy_move(void *dest, void *src)
{
    typedef typename yy_fix_type<T>::type TT;
...
template <class T> struct yy_auto_deleter
{
...
template <class T> class yy_storage
{
  private:
    typedef typename yy_fix_type<T>::type TT;
  ...
  public:
    typedef typename std::conditional<
        std::is_trivial<TT>::value, TT,
        typename std::aligned_storage<sizeof(TT), alignof(TT)>::type>::type
        type;
};
}
...
% %
    ...
    *unless * "lemon" is run with the "-m" command-line option.
...
#define yytestcase(X)
    ...
    ** follows
    **
    **   0 <= N <= YY_MAX_SHIFT             Shift N.  That is, push the
    *lookahead *                                      token onto the stack and
    *goto state N.
    **
    ...
    *yy_action[]. * Given state S and lookahead X, the action is computed as
    *either:
    **
...
#error "LEMON_SUPER must be defined."
    ...
    void yy_move(YYCODETYPE yymajor, YYMINORTYPE * yyDest,
    ...
    class yypParser : public LEMON_SUPER
    {
      public:
        ...
        template <class... Args> yypParser(Args &&... args);
        ...
        virtual int fallback(int iToken) const final override;
/*
** Return the peak depth of the stack for a parser.
*/
#ifdef YYTRACKMAXSTACKDEPTH
        int yypParser::stack_peak() { return yyhwm; }
#endif

#ifdef YYCOVERAGE
        int coverage(FILE *out);
#endif
      ...
      private:
        ...
        YYACTIONTYPE yy_find_shift_action(YYCODETYPE iLookAhead,
                                          YYACTIONTYPE stateno) const;
        YYACTIONTYPE yy_find_reduce_action(YYACTIONTYPE stateno,
                                           YYCODETYPE iLookAhead) const;

        void yy_shift(YYACTIONTYPE yyNewState, YYCODETYPE yyMajor,
                      ParseTOKENTYPE &&yypMinor);
#ifdef YYERRORSYMBOL
        void yy_shift_error(YYACTIONTYPE yyNewState);
#endif
        YYACTIONTYPE yy_reduce(unsigned int yyruleno, int yyLookahead,
                               const ParseTOKENTYPE &yyLookaheadToken);
        void yyStackOverflow();

#ifndef NDEBUG
        void yyTraceShift(int yyNewState, const char *zTag) const;
#else
#define yyTraceShift(X, Y)
#endif

#ifdef YYCOVERAGE
        mutable unsigned char yycoverage[YYNSTATE][YYNTOKEN] = {};
#endif
        ...
        int yyidx() const { return (int)(yytos - yystack); }
    };
    ...
    */
    void yy_destructor(YYCODETYPE yymajor, /* Type code for object to destroy */
     ...
     *
     */
    void yy_move(YYCODETYPE yymajor,    /* Type code for object to move */
                 YYMINORTYPE * yyDest,  /*  */
                 YYMINORTYPE * yySource /*  */
                 )
    {
    ...
    */
    ...
    template <class... Args>
    yypParser::yypParser(Args && ... args)
        : LEMON_SUPER(std::forward<Args>(args)...)
    {
#if YYSTACKDEPTH <= 0
        if (yyGrowStack())
        {
...
*/
    ...
    int yypParser::coverage(FILE * out)
    {
    ...
    */
    YYACTIONTYPE yypParser::yy_find_shift_action(
        ...
        do
        {
            ...
            if (yy_lookahead[i] != iLookAhead)
            {
...
#ifdef YYWILDCARD
                {
                    ...
                    assert(j < (int)(sizeof(yy_lookahead) /
    ...
    ** look-ahead token iLookAhead.
    */
    YYACTIONTYPE yypParser::yy_find_reduce_action(
        YYACTIONTYPE stateno, /* Current state number */
        YYCODETYPE iLookAhead /* The look-ahead token */
        ) const
    {
...
*/
#ifndef NDEBUG
    void yypParser::yyTraceShift(int yyNewState, const char *zTag) const
    {
        if (yyTraceFILE)
        {
            if (yyNewState < YYNSTATE)
            {
                fprintf(yyTraceFILE, "%s%s '%s', go to state %d\n",
                        yyTracePrompt, zTag, yyTokenName[yytos->major],
                        yyNewState);
            }
            else
            {
    ...
    */
        ...
        )
    {
        ...
        // yy_move<ParseTOKENTYPE>(std::addressof(yytos->minor.yy0),
        // std::addressof(yyMinor));
        yy_constructor<ParseTOKENTYPE>(std::addressof(yytos->minor.yy0),
                                       std::move(yyMinor));
    ...
    ** follow the reduce.
    */
    YYACTIONTYPE yypParser::yy_reduce(
        unsigned int yyruleno, /* Number of the rule by which to reduce */
        int yyLookahead,       /* Lookahead token, or YYNOCODE if none */
        const ParseTOKENTYPE
        ...
        )
    {
        ...
        ** enough on the stack to push the LHS value */
        if (yyRuleInfoNRhs[yyruleno] == 0)
        {
            ...
            {
                yyStackOverflow();
                /* The call to yyStackOverflow() above pops the stack until it
                *is * empty, causing the main parser loop to exit.  So the
                *return value * is never used and does not matter. */
                return 0;
            }
#else
            if (yytos >= &yystack[yystksz - 1])
            {
                ...
                {
                    yyStackOverflow();
                    /* The call to yyStackOverflow() above pops the stack until
                    *it is * empty, causing the main parser loop to exit.  So
                    *the return value * is never used and does not matter. */
                    return 0;
                }
                yymsp = yytos;
            }
...
*/
    ...
    */
    ...
    void yypParser::parse(int yymajor, /* The major token code number */
                          ...
                          )
    {
        ...
        do
        {
            ...
            {
                yy_shift(yyact, (YYCODETYPE)yymajor, std::move(yyminor));
#ifndef YYNOERRORRECOVERY
                yyerrcnt--;
#endif
                break;
...
// yyminorunion.yy0 = yyminor;
                ...
                */
                ...
                /* If the YYNOERRORRECOVERY macro is defined, then do not
                *attempt to * do any kind of error recovery.  Instead, simply
                *invoke the syntax * error routine and continue going as if
                *nothing had happened.
                **
                ** Applications can set this macro (for example inside %include)
                *if * they intend to abandon the parse upon the first syntax
                *error seen.
                */
                yy_syntax_error(yymajor, yyminor);
                // yy_destructor(yyminor);
    ...
    bool yypParser::will_accept() const
    {
        ...
        YYACTIONTYPE yyact;
        const YYCODETYPE yymajor = 0;
        ParseTOKENTYPE yyminor;
        std::vector<stack_entry> stack;

        // copy stack to stack.
        stack.reserve(yyidx() + 1);
        std::transform(begin(), end(), std::back_inserter(stack),
                       [](const yyStackEntry &e) {
                           return stack_entry({e.stateno, e.major});
                       });

        do
        {
            yyact = yy_find_shift_action(yymajor, stack.back().stateno);
            ...
            if (yyact >= YY_MIN_REDUCE)
            {
                ...
                YYCODETYPE yygoto = yyRuleInfoLhs[yyruleno];
                int yysize =
                    -yyRuleInfoNRhs[yyruleno]; /* stored as negative value */

                while (yysize--)
                    stack.pop_back();

                yyact = yy_find_reduce_action(stack.back().stateno, yygoto);
