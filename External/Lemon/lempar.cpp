...
void ParseInit(void *yypRawParser ParseCTX_PDECL)
{
    ...
    ParseCTX_STORE
// std::memset(pParser, 0, sizeof(yyParser)); /* not safe if extra_argument is
// not POD */
#ifdef YYTRACKMAXSTACKDEPTH
        yypParser->yyhwm = 0;
#endif
...
*/
static YYACTIONTYPE
yy_find_reduce_action(YYACTIONTYPE stateno, /* Current state number */
    ...
    if (yyTraceFILE)
    {
        if (yyNewState < YYNSTATE)
        {
            fprintf(yyTraceFILE, "%s%s '%s', go to state %d\n", yyTracePrompt,
                    zTag, yyTokenName[yypParser->yytos->major], yyNewState);
        }
        else
        {
                     ...
                     )
{
    ...
    // std::addressof(yyMinor));  yy_move also calls destructor.
    yy_constructor<ParseTOKENTYPE>(std::addressof(yytos->minor.yy0),
                                   std::forward<ParseTOKENTYPE>(yyMinor));
    yyTraceShift(yypParser, yyNewState, "Shift");
}

#ifdef YYERRORSYMBOL
static void
yy_shift_error(yyParser *yypParser,    /* The parser to be shifted */
               YYACTIONTYPE yyNewState /* The new state to shift in */
               )
{
    yyStackEntry *yytos;
...
yy_reduce(yyParser *yypParser,   /* The parser */
          ...
          const yy_fix_type<ParseTOKENTYPE>::type
...
static void yy_syntax_error(yyParser *yypParser, /* The parser */
                            ...
                            yy_fix_type<ParseTOKENTYPE>::type
           ...
           )
{
    ...
    do
    {
        ...
        {
            yy_shift(yypParser, yyact, (YYCODETYPE)yymajor,
                     std::forward<ParseTOKENTYPE>(yyminor));
#ifndef YYNOERRORRECOVERY
            yypParser->yyerrcnt--;
#endif
